...
# Under KDE Plasma, cursors are handled via system settings and typically
# work correctly with the nouveau driver. This script adapts behavior
# based on the detected desktop environment.
...
if [[ ! -f $nvidia_config ]] &&
  ...
  if [[ -n "${XDG_CURRENT_DESKTOP:-}" ]] && [[ "$XDG_CURRENT_DESKTOP" == *"KDE"* ]]; then
    echo "Detected nouveau driver under KDE Plasma. Hardware cursors should work natively."
    echo "If the pointer is invisible, check System Settings > Display > Cursor."
    return 0
  fi

  # Hyprland fallback: patch looknfeel.lua
